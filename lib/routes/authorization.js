  ...
  handler: function authorizationEndpoint(req, reply) {
    ...
    P.all([
      verify(req.payload.assertion).then(function(claims) {
        ...
        // Check to see if the acr value requested by oauth matches what
        const acrValues = req.payload.acr_values;
        if (acrValues) {
          const acrTokens = acrValues.split(' ');
          if (acrTokens.include(ACR_VALUE_AAL2) && claims['fxa-aal'] !== 2) {
            throw AppError.mismatchAcr(acrValues, claims['fxa-aal']);
          }
        }
